...
for num, filename in enumerate(binaryFile):
    ...
    scaleValue = scale[num]                # scaleSNR
    binData = readBinary(filename, simBinaryNchan, simNsamp)
    arrayStart = int(pulseOffset[num]/tbin)
    arrayEnd = arrayStart + simNsamp
    scaleCONT = 0.025
    scaleSubint = int(arrayStart/nsblk)
    scaleData = fits[1].read(rows=[scaleSubint], columns=['DATA'])
    scaleBandpass = scaleData[0][0][:,0,:,0].sum(axis=0)/nsblk
    scaleSTD = np.std(scaleBandpass)
    print "+"*50
    print "Reading spectra shape: ", specFile[num], "Scale Value= SNR(", scaleValue, ") * RMS(", round(scaleSTD,3), ") * Cont(", scaleCONT,")"
    print "Reading simulation data: ", num, filename, "Pulse TOA= ", pulseOffset[num], "sec"
 
    # simulate pulse * SNR * scaleCONT * scaleSTD * frequence shape
    simdata[arrayStart:arrayEnd, :] = binData*scaleValue*scaleSTD*scaleCONT*specshape + simdata[arrayStart:arrayEnd, :]
